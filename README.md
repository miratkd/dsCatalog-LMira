# DsCatalog (eng-USA)
### A simple way to show all your products to your customers! 
...
A full-stack project for an online catalog developed from scratch during the BootCamp DevSuperior. The site allows a business owner to display his products on the Internet in an organized and detailed way, in addition to registering employees who will have full or partial access to the system. The products will be presented through a simple and beautiful interface, available for both desktop and mobile.

# Layot

being the most important part of the project, the catalog page presents all products in a paginated and filtered way. The user has the option of navigating between pages to check the entire catalog of products, one by one, or use the filter products to find exactly what he is looking for. 

![Catalogo](https://github.com/miratkd/assets/blob/main/catalogo.PNG)

Filtering is quite simple and basically consists of two functions, filtering by name and filtering by category. When filtering by name, the user types a product name and the screen will only show a product with that name in the title. In filtering by category, the user has to choose between the categories already registered in the system and only the products associated with that category will be shown on the screen. There is also a button to clear all filters and show all products. 

![Filtro](https://github.com/miratkd/assets/blob/main/Catalogo-livros.PNG)

It is also important to remember that all of these functions are available in the mobile version too! 
...
![catalogo-mobile](https://github.com/miratkd/assets/blob/main/Catalogo-mobile.PNG)
 ...
 The entire site administration system is protected by a robust authentication system, ensuring that only registered users can access specific resources 
 
  ![Login](https://github.com/miratkd/assets/blob/main/Login.PNG)
  
In the admin session, system operators can add, remove or edit products and categories, allowing the catalog to be always up to date. All site data is saved in a postgreSQL database hosted along with the backend on the heroku platform. 
...
There are two types of system operators; the "operators", those who can add and remove products and categories, and the "admins", those who can add and remove other operators 
 ...
 Note that the option to edit users appears only for users of the admin type. 
...
Sendo talvez a parte mais importante do projeto, a página de catálogo apresenta todos os produtos de forma paginada e filtrada. O usuário tem a opção de ir navegando entre
 ...
 . A também um botão para limpar todos os filtros e mostrar todos os produtos.
